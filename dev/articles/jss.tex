...
method. Using causal graphical models, this can be used to address
hypothetical questions of the form ``What would my salary have been, had
I been of a different gender/race?''. Such counterfactual reasoning can
help eliminate discrimination and help justify fair decisions.
}
...
\citep{blau2003pay, lambrecht2019algorithmic}, and this realization
...
classifier or regressor \(\widehat{Y}\). Another notion, termed
...
for a predictor \(\widehat{Y}\) simultaneously. There is no consensus on
which notion of fairness is the correct one.
...
Examples for this include counterfactual reasoning
...
affect the probability of a specific candidate being accepted for given
job opening. This approach has motivated another notion of fairness,
termed \emph{counterfactual fairness} \citep{kusner2017counterfactual},
which states that the decision made, should remain fixed, even if,
hypothetically, some parameters such as race or gender were to be
changed (this can be written succinctly as
\(\widehat{Y}(a) = \widehat{Y}(a')\) in the potential outcomes
notation). Causal inference can also be used for furthering
understanding of demographic parity by decomposition of the parity gap
measure,
\(\mathbbm{P}(\widehat{Y} = 1 \mid A = a) - \mathbbm{P}(\widehat{Y} = 1 \mid A = a')\),
into the direct, indirect and spurious components, as well as the
...
CRAN. Currently there are only few packages being distributed via CRAN,
that are related fair machine learning, including \pkg{fairml}
...
implementations of the developed methods are lagging behind.
...
protected attribute, describing candidate gender, where \(A = a\)
...
indicate how variables affect one another.

\begin{CodeChunk}
\begin{figure}

{\centering \includegraphics[width=0.5\linewidth]{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/Rtmp0rywIx/file267777fa2fef/dev/articles/jss_files/figure-latex/uni-adm-1} 
...
value than 60\% of males in the dataset, we assume that he would also be
better than 60\% of females had he been female\footnote{This assumption
...
{\centering \includegraphics{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/Rtmp0rywIx/file267777fa2fef/dev/articles/jss_files/figure-latex/rel-edu-1} 

}

\caption[TODO]{TODO: Add a figure caption.}\label{fig:rel-edu}
\end{figure}
\end{CodeChunk}
...
\emph{structural causal model} (SCM) can be constructed, which fully
encodes the assumed data-generating process. An SCM is represented by a
4-tuple \(\langle V, U, \mathcal{F}, P(u) \rangle\), where
...
of university admission data on students. We subset this data, using the
first \texttt{n\_samp} rows as training data (\texttt{uni\_trn}) and the
following \texttt{n\_samp} rows as testing data (\texttt{uni\_tst}).
Furthermore, we construct an adjacency matrix \texttt{uni\_adj} with
edges \(\texttt{gender} \to \texttt{edu}\),
...
\texttt{graphModel()} turns a graph specified as adjacency matrix into
an annotated graph using the \pkg{igraph} package
...
{\centering \includegraphics[width=0.6\linewidth]{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/Rtmp0rywIx/file267777fa2fef/dev/articles/jss_files/figure-latex/graph-plot-1} 

}

\caption{The underlying graphical model corresponding to the university admission example (also shown in Figure \ref{fig:uni-adm}. Vertex labels are abbreviated using the first two letters of the respective variable names. The vertex \textit{ge} therefore represents \texttt{gender}, etc.}\label{fig:graph-plot}
\end{figure}
\end{CodeChunk}
...
both represent the same causal model. For plotting purposes, we use the
fist two letters to denote vertex labels (i.e., vertex \emph{ge}
represents candidate gender (the \texttt{gender} column of
\texttt{uni\_dat}) and is equivalent to vertex \emph{A} in Figure
\ref{fig:uni-adm}, representing the protected attribute.
...
calling \texttt{predict()} on the returned \texttt{fairadpat} object in
order to perform data adaption on new test data).
...
conceivable and therefore \pkg{fairadpat} provides an extension
...
S3-classed object, for which an implementation of the S3 generic
function \texttt{computeQuants()} is available.
...
As a possible real-world use of \pkg{fairadapt}, suppose that after a
...
form as package dataset from \pkg{fairadapt}. Columns are grouped into
...
matrix \texttt{gov\_adj}:

\begin{CodeChunk}
\begin{figure}

{\centering \includegraphics[width=0.5\linewidth]{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/Rtmp0rywIx/file267777fa2fef/dev/articles/jss_files/figure-latex/census-tikz-1} 
...
{\centering \includegraphics[width=0.9\linewidth]{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/Rtmp0rywIx/file267777fa2fef/dev/articles/jss_files/figure-latex/census-graph-1} 
...
{\centering \includegraphics{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/Rtmp0rywIx/file267777fa2fef/dev/articles/jss_files/figure-latex/before-adapt-1} 
...
using the \pkg{ggplot2}-exported S3 generic function
\texttt{autoplot()}:
...
{\centering \includegraphics{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/Rtmp0rywIx/file267777fa2fef/dev/articles/jss_files/figure-latex/vis-adapt-1} 
...
descendant of the protected attribute \(A\) (\texttt{sex}, cf., Figure
\ref{fig:census-graph}). However, variables \texttt{education\_level}
and \texttt{salary} do change for males, as they are descendants of
\(A\).
...
This is the idea behind \emph{resolving variables}, discussed in the
following section (\ref{adding-resolving-variables}).
...
the the idea of \emph{resolving variables} by
...
resolving in the \texttt{uni\_admission} dataset used in
...
variation in this case is larger than in the \texttt{basic} example from
Section \ref{implementation}, with no resolvers. The intuitive reasoning
here is that resolving variables allow for some discrimination, so we
expect to see a larger total variation between the groups.
...
{\centering \includegraphics[width=0.6\linewidth]{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/Rtmp0rywIx/file267777fa2fef/dev/articles/jss_files/figure-latex/res-graph-1} 

}

\caption[Visualization of the causal graph corresponding to the university admissions example introduced in Section \ref{introduction} with the variable \texttt{test} (\textbf{te}) chosen as \textit{resolving variable} and therefore highlighted in red]{Visualization of the causal graph corresponding to the university admissions example introduced in Section \ref{introduction} with the variable \texttt{test} (\textbf{te}) chosen as \textit{resolving variable} and therefore highlighted in red.}\label{fig:res-graph}
\end{figure}
\end{CodeChunk}
...
latent variables corresponding to variable test and final score be
...
{\centering \includegraphics[width=0.5\linewidth]{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/Rtmp0rywIx/file267777fa2fef/dev/articles/jss_files/figure-latex/semi-markov-1} 

}

\caption{Causal graphical model corresponding to a Semi-Markovian variant of the university admissions example, introduced in ef{implementation} and visualized in its basic form in Figures \ref{fig:uni-adm} or \ref{fig:graph-plot}. Here, we posit a mutual dependency between the variables representing test and final scores.}\label{fig:semi-markov}
\end{figure}
\end{CodeChunk}
...
matrix \texttt{uni\_adj} and set entries representing edged between
vertices \texttt{test} and \texttt{score} to \(1\). Finally, we can pass
this confounding matrix as \texttt{cfd.mat} to \texttt{fairadapt()}. A
visualization of the resulting causal graph is available from Figure
\ref{fig:semi-graph}.
...
{\centering \includegraphics[width=0.6\linewidth]{/private/var/folders/24/8k48jl6d249_n_qfxwsl6xvm0000gn/T/Rtmp0rywIx/file267777fa2fef/dev/articles/jss_files/figure-latex/semi-graph-1} 

}

\caption{Visualization of the causal graphical model also shown in Figure \ref{fig:semi-markov}, obtained when passing a confounding matrix indicating a bidirectional edge between vertices \texttt{test} and \texttt{score} to \texttt{fairadapt()}. The resulting Semi-Markovian setting can be handles by \texttt{fairadapt()}, extending the basic Markovian formulation introduced in Section \ref{markovian-scm-formulation}.}\label{fig:semi-graph}
\end{figure}
\end{CodeChunk}

Alternatively, instead of using the extended parent set
\(\mathrm{Pa}(V_i)\), we could also use the \emph{largest possible} set
of parents, i.e., the ancestors \(\mathrm{an}(V_i)\). This approach is
implemented as well, and available by specifying a topological ordering.
This is achieved by passing a \texttt{character} vector, containing the
correct ordering of the names appearing in \texttt{names(train.data)} as
\texttt{top.ord} argument to \texttt{fairadapt()}.

\hypertarget{questions-of-identifiability}{%
\subsection{Questions of
Identifiability}\label{questions-of-identifiability}}

So far, the question whether it is always possible to carry out the
...
intervention is identifiable if any of the following conditions hold:
...
\item
  The model is Semi-Markovian and there is no bidirected path between
  \(A\) and \(\mathrm{ch}(A)\).
\item
  The model is Semi-Markovian and there is no bidirected path between
  \(R_i\) and \(\mathrm{ch}(R_i)\) for any resolving variable \(R_i\).
\end{itemize}
...
but we hope to include this in a future version.
