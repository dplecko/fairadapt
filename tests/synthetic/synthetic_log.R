...
getplots <- function(info, ResSet, label){
  ...
  override.shape <- c(4,8,15,16,17,18,21,22, 23)
  shape <- 15:22
  ...
  p1 <- ggplot(data = frame, aes(x = tv0, y = auc0)) +
    ...
    scale_shape_manual(values=c(15:20, 24, 25, 23)) +
    ...
    theme_bw() +
    theme(legend.position = c(0.8,0.3), legend.box.background = element_rect(color = "black"))
  ...
  p2 <- ggplot(data = frame, aes(x = tv0, y = cs0)) +
    ...
    ggtitle(paste0("Synthetic ", label, " - discrimination removal with resolvers"), subtitle = waiver()) +
    theme_bw() +
    theme(legend.position = c(0.8,0.65),legend.box.background = element_rect(color = "black"))
  return(list(p1,p2))
}
...
GAUCS <- function(adj.mat, res.list, GenMech, prot.attr = "A", nrep = 10,
  ...
  for(i in 1:nrep){
    ...
    for(j in 1:length(res.list)){
      ...
      adapted.train.data$Y <- factor(as.integer(adapted.train.data$Y))
      adapted.train.data$A <- NULL
      LR <- stats::glm(Y ~ ., family = "binomial", data = adapted.train.data)
      S.hat <- predict(LR, newdata = adapted.test.data, type = "response")
...
# Synthetic example 2
{
  ...
  ResolvingLevelGenB <- function(n) {
    ...
    Y <- rbinom(n, size = 1, prob = expit((X1+X2+X3+sigma)))
...
save.image(file = "synthetic_log.RData")
quit()

load(file.path(root, "tests", "synthetic", "synthetic_log.RData"))
# python comparisons run locally
{
  ...
  infoA <- c(infoA, addA[1])
  infoB <- c(infoB, addB[1])
}
...
# plotting offline
{
  ResSet <- factor(c(as.character(ResSetA), "reweighing"))
  x <- getplots(infoA, ResSet, "A")
  cowplot::plot_grid(x[[1]], x[[2]], nrow = 2L)
  ggsave(paste0(file.path(root, "..", "Article", "syntheticA"), ".png"),
    device = "png", width = 7, height = 7)
  
  ResSet <- factor(c(as.character(ResSetB), "reweighing"))
  x <- getplots(infoB, ResSet, "B")
  cowplot::plot_grid(x[[1]], x[[2]], nrow = 2L)
