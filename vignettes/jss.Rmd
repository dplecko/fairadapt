...
<!-- As a side-remark: you have in your print() output the `Call:` entry,
...
```{r, graph-plot, echo = FALSE, fig.width = 6, fig.height = 3, fig.cap = "The underlying graphical model in the university admission example (also shown in Figure \\ref{fig:uni-adm}. Vertex labels are abbreviated using the first two letters of the corresponding variable names. The vertex *ge* therefore represents `gender`, etc.", out.width = "60%"}
...
A visualization of the `igraph` object returned by `graphModel()` is available from Figure \ref{fig:graph-plot}. The graph shown is equivalent to that of Figure \ref{fig:uni-adm} as they both represent the same causal model. For plotting purposes, we use the fist two letters to denote vertex labels (i.e., vertex *ge* represents candidate gender and is equivalent to vertex *A* in Figure \ref{fig:uni-adm}, representing the protected attribute.
...
The university admission example presented in Section \ref{methodology} demonstrates how we can compute counterfactual values for an individual while preserving their relative educational achievement. In particular, for a male student with values $(a, e, t, y)$, we compute his "fair-twin" values $(\tilde{a}, \tilde{e}, \tilde{t}, \tilde{y})$ - the values the student would have obtained, had he been female. To explicitly compare a person to their hypothetical fair-twin, we use the `fairTwins()` function, applied to an object of class `"fairadapt"`:

```{r, fairtwin-uni, include = T}
fairTwins(basic, train.id = 1:5)
```
...
<!-- TODO: i added a bit of explanation as to remove comments in code -->
...
<!-- TODO: for visual reasons, i turned this into 3 top-level bullet points;
     undo if you feel it was better before -->
