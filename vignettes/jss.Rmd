...
\begin{tikzpicture}
  ...
  \begin{scope}
    ...
    \draw [very thin] (0*\sx, 0*\sy) -- (0*\sx, 5.5*\sy);
		...
		\node[rotate=90] () at (0.75*\sx, 2.875*\sy) {DP};
    \node[rotate=90] () at (0.75*\sx, 4.625*\sy) {EO};
    ...
    \fill [pattern={Lines[angle=45,distance=5pt], very thin},pattern color=black] 
      (3*\sx, 3.75*\sy) rectangle (5*\sx, 5.5*\sy);
    \fill [pattern={Lines[angle=45,distance=5pt], very thin},pattern color=black] 
      (7*\sx, 3.75*\sy) rectangle (9*\sx, 5.5*\sy);
    ...
    \node [s1] () at (2*\sx, 0.75*\sy) {fairness};
    \node [s1] () at (4*\sx, 1.25*\sy) {fairmodels};
...
```{r benchmark-quant-methods, echo = FALSE}
...
if (quick_build && file.exists(bm_cache)) {
  ...
  bmark <- lapply(
    ...
    \(quant.method) {
      ...
      names(res) <- as.character(nsamps)
      res
    }
  )

  names(bmark) <- c("rangerQuants", "mcqrnnQuants", "linearQuants")
...
\midrule
    ...
    \addlinespace[0.3em]
    $T(200, 4)$ & `r bmark[["rangerQuants"]][["200"]]` sec & `r bmark[["mcqrnnQuants"]][["200"]]` sec & `r bmark[["linearQuants"]][["200"]]` sec \\
    \addlinespace[0.3em]
    $T(500, 4)$ & `r bmark[["rangerQuants"]][["500"]]` sec & `r bmark[["mcqrnnQuants"]][["500"]]` sec & `r bmark[["linearQuants"]][["500"]]` sec \\
\bottomrule
\end{tabular}
...
Using `rangerQuants()` and `mcqrnnQuants()` utilizes flexible machine learning tools that have a number of parameters. It is possible to optimize the fitting procedure by tuning these parameters. However, when doing so, we need a way of assessing the quality of our fit. In the context of quantile regression we can estimate the expected $\tau$-quantile loss function, 
...
\begin{enumerate}[label=(\roman*)]
  \item for \texttt{ranger()}: parameters \texttt{mtry} (number of candidate variables in each split), \texttt{min.node.size} (size of leaf nodes after which splitting ) and \texttt{max.depth} (maximum depth of each tree),
...
\end{enumerate}
Optimizing the quantile fit should be of interest mostly to advanced users (and hence we do not perform parameter tuning explicitly here). The basic user is strongly advised to work with default values, which are often a good starting point.
...
The first, commonly encountered source of uncertainty is the one induced by a finite sample size. The training data $\mathcal{D}_{train}$ has a finite size, and for this reason inferences made using this data are imperfect. We wish to quantify the error in the predictions $\widehat{Y}^{fair}_{test}$ introduced by the finite sample size of $\mathcal{D}_{train}$. As Figure \ref{fig:workflow-tikz} shows, the training data $\mathcal{D}_{train}$ affects the resulting fair predictions in two ways. Firstly, it affects the value of the transformed test data $\widetilde{\mathcal{D}}_{test}$ (mediated by the fairadapt S3 object). Secondly, it affects the predictions $\widehat{Y}^{fair}_{test}$ also through the predictor (since it is the input to the regressor classifier). These finite sample uncertainties can be analyzed using \textit{bootstrap} \citep{efron1994introduction}. This means that we repeat the procedure in Figure \ref{fig:workflow-tikz} many times, each time taking a different bootstrap sample of the training data. Below we will show how this can be done with the `fairadaptBoot()` function. 
...
For quantifying uncertainty, we use the `fairadaptBoot()` function, whose most important arguments are:
  ...
  - `keep.object`, a `logical` scalar, indicating whether all the `fairadapt` S3 objects built in bootstrap repetitions should be kept in memory. Default value is `FALSE`.
...
FA.boot.fin <- fairadaptBoot(two_year_recid ~ ., "race", cmp_mat, cmp_trn,
                             cmp_tst, rand.mode = "finsamp", n.boot = n_itr)
```
...
FA.boot.quant <- fairadaptBoot(two_year_recid ~ ., "race", cmp_mat, cmp_trn,
                               cmp_tst, rand.mode = "quant", n.boot = n_itr)
```
