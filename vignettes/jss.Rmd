...
We note that `train.data` and `test.data` need to have column names which appear in the names of the adjacency matrix `colnames(adj.mat)`. The protected attribute $A$ is given as a character vector `protect.A` of length one.
...
\begin{table}
  ...
  \texttt{quant.method} argument & `rangerQuants` & `mcqrnnQuants` & `linearQuants` \\ \hline
