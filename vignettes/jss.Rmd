...
\begin{enumerate}[label=(\arabic*)]
  \item *demographic parity* \citep{darlington1971fairness} requires the protected attribute $A$ (gender/race/religion etc.) to be independent of a constructed classifier or regressor $\widehat{Y}$, written as $$\widehat{Y} \ci A,$$
  \item *equality of odds* \citep{hardt2016eosl}, requires equal false positive and false negative rates of classifier $\widehat{Y}$ between different groups (females and males for example) written as $$\widehat{Y} \ci A \mid Y,$$
  \item *calibration* 
\end{enumerate}
...
In Figure \ref{fig:software} we give a graphical overview of some of the available software in \proglang{python} and \proglang{R}, which are the languages most commonly used in fairness analysis. The figure might serve as a guide to a new fairness analyst wishing to get started. In particular, we note that bias detection software usually covers a wide range of different metrics, compatible with different fairness definitions. When it comes to bias removal, not all methods are compatible with all the definitions ... . Furthermore, some methods might specifically work in the classification or regression setting. Therefore, in the bias removal task we indicate which methods/software supports which type of definition and prediction problem.

```{tikz, software, fig.cap = "Software options for fair data analysis in \\proglang{R} and \\proglang{python}. In the left column, nodes correspond to \\proglang{R}-packages and in the right column to \\proglang{python} modules.", fig.ext = "png", cache = TRUE, echo = FALSE, out.width = "100%"}
...
\begin{tikzpicture}
  ...
  \begin{scope}
     \draw[draw=black, very thin] (1*\sx, 7.25*\sy) rectangle ++(8*\sx, -0.5*\sy);
		...
		\draw [very thin] (9*\sx, 2.5*\sy) -- (9.5*\sx, 2.5*\sy);
		\draw [very thin] (9*\sx, 3.5*\sy) -- (9.5*\sx, 3.5*\sy);
		\draw [very thin] (9*\sx, 4.5*\sy) -- (9.5*\sx, 4.5*\sy);
		\draw [very thin] (9*\sx, 5.5*\sy) -- (9.5*\sx, 5.5*\sy);
		\draw [very thin] (9.5*\sx, 2.5*\sy) -- (9.5*\sx, 5.5*\sy);

		\node[rotate=-90] () at (9.25*\sx, 3*\sy) {DP};
        \node[rotate=-90] () at (9.25*\sx, 4*\sy) {calib};
        \node[rotate=-90] () at (9.25*\sx, 5*\sy) {EO};
		

		\draw [thin, dotted] (\sx,2.5 * \sy) -- (9 * \sx, 2.5*\sy);


		\node [draw=red, rotate=90] at (0.75*\sx, 1.5*\sy)  {Bias Detection};
  		\node [draw=red, rotate=90] at (0.75*\sx, 4*\sy)  {Bias Removal};
        ...
        \node [f1] () at (8*\sx, 4*\sy) {aif360};
        \node [f1] () at (6*\sx, 5*\sy) {fairness};
        \node [f1] () at (8*\sx, \sy) {parity\\fairness};
        \node [f1] () at (8*\sx, 2*\sy) {fairness\\indicators};
        \node [f1] () at (6*\sx, 2*\sy) {ethicML};
        \node [f1] () at (6*\sx, 3*\sy) {fairlearn};
        

        \node [f1] () at (4*\sx, 2*\sy) {fairadapt};
        \node [f1] () at (2*\sx, 2*\sy) {fairml};
        \node [f1] () at (4*\sx, 4*\sy) {fairness};
        \node [f1] () at (2*\sx, 4*\sy) {fairmodels};
  \end{scope}
...
Python cosmos
  - describe fairlearn and aif360 as larger
R-cosmos
...
The second source of uncertainty arises from the uncertainty in quantiles and is specific to \pkg{fairadapt}. As described in Section \ref{methodology} (see also Figure \ref{fig:rel-edu}), the fairadapt procedure aims to preserve the relative quantile of the variable, when computing the $do(A=a)$ intervention. However, when we are working with variables that are not continuous, defining a quantile precisely becomes more difficult\footnote{Think of a case where we have a binary $X \in \lbrace 0, 1 \rbrace$. In such a case, it is impossible to define what a 70\% quantile is, as opposed to the continuous case (of a Gaussian variable $X$ for example).}. Therefore, in presence of discrete variables, due to the imperfect estimation of quantiles, the fairadapt procedure has some inherent randomness. This randomness would still persist even if we had infinite training samples in $\mathcal{D}_{train}$. Importantly, to achieve fair predictions, taking an expectation over this randomness is not feasible. For a detailed discussion of why this is the case, we refer the reader to \citet[Section~5]{plecko2020fair}.

For quantifying uncertainty, we use the `fairadaptBoot()` function, whose most important arguments are:
  
  - `formula`, `prot.attr`, `adj.mat`, `train.data` as for the `fairadapt()` function (see Section \ref{implementation})
  - `test.data`, `data.frame` containing the test data, defaults to `NULL`. Whenever the test data equals `NULL`, then `save.object` must be `TRUE`.
  - `save.object`, a `logical` scalar, indicating whether all the fairadapt S3 objects built in boostrap repetitions should be saved. Default value is `FALSE`.
  - `rand.mode`, a `character` scaler, taking values `"finsamp"`, `"quant"` or `"both"`, corresponding to considering finite sample uncertainty, quantile uncertainty or both.
  
The function `fairadaptBoot()` returns an S3 object of class `fairadaptBoot`. We emphasize that calling this function can be computationally heavy, both in terms of runtime and memory. Setting the `save.object` argument to be `FALSE` reduces the memory consumption substantially, with the drawback that `test.data` has to be provided in the argument, and also that the resulting `fairadaptBoot` object cannot be reused. Setting the `save.object` to `TRUE` might consume a lot of memory, but then this object can be reused over again for transforming the test data. This can be done by using the `predict()` function on the `fairadaptBoot` S3 object, to which a `test.data` argument is available.

We now compare the finite sample uncertainty and the quantile uncertainty on the COMPAS dataset \citep{larson2016compas}. We begin by loading the COMPAS dataset and constructing its causal graph:
```{r, compas}
...
head(cmp_dat)
```
The COMPAS dataset contains information on 7214 individuals from Broward County, Florida, who were released on parole. The variables include race, sex, age, juvenile offense counts, priors count and the charge of the degree. The outcome of interest is recidivism within two years and the prottected attribute is race (taking values Non-White and White). A possible causal graph for the COMPAS dataset is given in Figure \ref{fig:compas-graph}.

```{tikz, compas-graph, fig.cap = "The causal graph for the COMPAS dataset. $Z$ are demographic features, $A$ is race, $J$ juvenile offense counts, $P$ priors count, $D$ the degree of charge, and $Y$ two year recidivism.", fig.ext = "png", cache = TRUE, echo = FALSE, out.width = "50%"}
...
After loading the dataset, we run the `fairadaptBoot()` function twice, with two different values of the `rand.mode` argument. Firstly, we run a boostrap with only the finite sample uncertainty. To simplify, we also train a random forest classifier on the entire training data and consider the classifier as fixed.
...
FA.boot.fin <- fairadaptBoot(two_year_recid ~ ., "race", cmp_mat, cmp_trn, 
                             cmp_tst, rand.mode = "finsamp", n.boot = 3L)
```
Then, we run the bootstrap procedure, but by considering only the inherent quantile uncertainty.
```{r, compas-boot-quant, cache=TRUE}
FA.boot.quant <- fairadaptBoot(two_year_recid ~ ., "race", cmp_mat, cmp_trn, 
                               cmp_tst, rand.mode = "quant", n.boot = 3L)
```
The objects we obtained are of class `fairadaptBoot` and contain a list `adapt.test` which contains different replicates of the adapted test data (the length of this list in `n.boot`, the number of bootstrap repetitions). Finally, to obtain predictions, we train a random forest classifier on the original training data and apply it to all of the obtained transformed datasets:
...
df <- rbind(jac_frm(pred_fin, "Finite Sample"), jac_frm(pred_quant, "Quantiles"))
p1 <- ggplot(df, aes(x = tsh, y = y, color = mode)) +
  geom_point() + geom_line() +
  geom_ribbon(aes(ymin = y - sd, ymax = y + sd, fill = mode, color = mode), alpha = 0.3) +
...
ord_ind <- function(res, modes = "single") {
  ...
  if (is.list(res) & length(res) > 1L) return(Reduce(rbind, Map(ord_ind, res, modes)))
...
df <- rbind(ord_ind(pred_fin, "Finite Sample"), ord_ind(pred_quant, "Quantiles"))
p2 <- ggplot(df, aes(x = ret, color = mode)) +
  stat_ecdf() + theme_bw() + xlab("Probability of preserving ordering") +
  ylab("Cumulative proportion") + ggtitle("Order preservation of individuals") +
  theme(
...
3. Another interesting metric is that of inversion number. Notice that $p_A, p_B$ define two permutations of the $n_{test}$ individuals, when we considering sorting them according to their predicted probabilities. We can compute the inversion number of these two permutations $\pi_A, \pi_B$, which is the total number of pairs of individuals whose ordering is not the same in $\pi_A$ and $\pi_B$. Notice that the maximum value of the inversion number is $n_{test} \choose 2$. Hence, we normalize this quantity by $n_{test} \choose 2$.
...
df <- rbind(inv_frm(pred_fin, "Finite Sample"), inv_frm(pred_quant, "Quantiles"))
p3 <- ggplot(df, aes(x = ret, fill = mode)) +
  geom_density(alpha = 0.3) + xlim(0, 1) + theme_bw() +
...
df <- rbind(prb_frm(pred_fin, "Finite Sample"), prb_frm(pred_quant, "Quantiles"))
p4 <- ggplot(df, aes(x = -width, color = mode)) +
  stat_ecdf() + theme_bw() +
  xlab("95% CI width") + ylab("Cumulative probability") +
  theme(
    legend.position = c(0.2, 0.8),
    legend.box.background = element_rect()
  ) + ggtitle("Conf. Int. width")

```

The results of these four metric applied to the COMPAS dataset are shown in \ref{fig:compas-dm}.

```{r, compas-dm, echo = FALSE, fig.width = 8, fig.height = 6, fig.cap = "Analyzing uncertainty of predictions in the COMPAS dataset.", out.width = "100%"}
cowplot::plot_grid(
  p1, p2, p3, p4, ncol = 2L
)
```
...
```{r, indiv-uncq}
ind_prb <- data.frame(prob = as.vector(t(pred_fin[1:3, ])), 
                      individual = rep(c(1, 2, 3), each = FA.boot.fin$n.boot))
```

and is visualized in Figure \ref{fig:compas-indiv}. 

```{r, compas-indiv, echo = FALSE, fig.width = 8, fig.height = 6, fig.cap = "Analyzing uncertainty of predictions in the COMPAS dataset.", out.width = "100%"}
ggplot(ind_prb, aes(x = prob, group = individual, fill = "black")) + 
